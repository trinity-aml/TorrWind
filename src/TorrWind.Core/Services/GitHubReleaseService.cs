...
public sealed class GitHubReleaseService
{
    ...
    private const string UserAgent = "TorrWind/0.1";
    ...
    private async Task<TorrServerRelease> GetTorrServerReleaseFromGitHubPageAsync(
        ...
        CancellationToken cancellationToken)
    {
        var asset = FindWindowsAmd64Asset(ParseReleaseAssets(releaseHtml));
        ...
        if (asset is null)
        {
            ...
            var (assetsHtml, _) = await GetGitHubHtmlAsync(
                    ...
                    cancellationToken)
                .ConfigureAwait(false);
            asset = FindWindowsAmd64Asset(ParseReleaseAssets(assetsHtml));
        }
        ...
        return new TorrServerRelease(version, asset.Name, asset.DownloadUrl, exactSize, publishedAt, false);
    }
    ...
    {
        var asset = FindWindowsAmd64Asset(release.Assets.Select(asset =>
            new ReleaseAsset(asset.Name, asset.BrowserDownloadUrl, asset.Size)));
    ...
    private sealed record ReleaseAsset(string Name, Uri DownloadUrl, long SizeBytes);
