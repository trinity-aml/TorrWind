...
- Provide field-based runtime settings for advanced official TorrServer `BTSets` options including retrackers mode, peer listen port, BT protocol toggles, cache drop behavior, Rutor/Torznab search toggles, responsive reader, playback timecode tracking, JSON storage flags, and P2P proxy hosts.
